...
- **Region 版**：你的组织部署在某个地域（如 `cn-hangzhou`）或自建实例，需要在偏好里切换"接入点 = Region 版"，并填入对应的 `Region 版 API Base URL`。该地址必须是带主机名的完整 HTTPS URL；Region 版的请求 path 不带 `organizations/{organizationId}/` 段。
