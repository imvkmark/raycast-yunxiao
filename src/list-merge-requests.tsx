...
import { diagnosticUrl, safeHttpsUrl } from "./utils/urls";
...
function mergeRequestUrl(request: MergeRequest): string | undefined {
    // 仓库路径不在响应里；只信 API 直接返回的 webUrl / detailUrl，缺则不打开。
    return safeHttpsUrl(request.webUrl) ?? safeHttpsUrl(request.detailUrl);
}
